...
export function CreateEntrevistaForm() {
  const [open, setOpen] = useState(false);
  const [state, action, pending] = useActionState(createEntrevista, undefined);
  ...
  return (
    ...
    >
      <h2 className="font-serif text-lg text-deep-water">Nueva entrevista</h2>
      ...
      <div className="space-y-1">
        <label htmlFor="motivo" className="font-sans text-xs font-medium text-ink">
          Motivo
        </label>
        <input
          id="motivo"
          name="motivo"
          placeholder="p. ej. Recomendo, integración, llamamiento…"
          required
          className="w-full rounded-xl border border-line px-3 py-2 font-sans text-sm outline-none focus:border-living-teal"
        />
        {state?.errors?.motivo && (
          <p className="font-sans text-xs text-rojo">{state.errors.motivo[0]}</p>
        )}
      </div>
